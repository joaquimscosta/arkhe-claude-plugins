...
```bash
# From the repo root
gemini extensions install ./.gemini-extensions/startup
```
...
mkdir -p ~/.codex/plugins/startup
cp .codex-marketplace/startup/AGENTS.md ~/.codex/plugins/startup/AGENTS.md
ln -s "$(pwd)/plugins/startup/skills" ~/.codex/plugins/startup/skills
```
