...
description: >-
  ...
  Use when user runs /devtools:claude-setup, mentions "setup claude code", "configure claude",
...
| 7 | Settings | Section 13 | Language, background tasks in settings.json |
   ...
   If user passed a specific category as argument (e.g., `/devtools:claude-setup hooks`), skip the selection and go directly to that category.
...
5. **Post-setup summary** — show what was configured, next steps, and remind user they can re-run `/devtools:claude-setup` for incremental updates.
