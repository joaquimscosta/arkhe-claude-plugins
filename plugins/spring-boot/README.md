...
```bash
# From the repo root
gemini extensions install ./.gemini-extensions/spring-boot
```
...
mkdir -p ~/.codex/plugins/spring-boot
cp .codex-marketplace/spring-boot/AGENTS.md ~/.codex/plugins/spring-boot/AGENTS.md
ln -s "$(pwd)/plugins/spring-boot/skills" ~/.codex/plugins/spring-boot/skills
```
...
Some commands in `spring-boot` dispatch Claude subagents (a Claude-only capability). On Gemini and Codex, those commands ship with the agent's prompt **inlined** and a banner:

> *On Gemini/Codex, the agent that would be invoked on Claude has been collapsed into this prompt. Behavior is degraded — no parallel sub-execution.*

Affected commands degrade gracefully for single-pass workflows (deep research, code review, debugging) but lose parallel sub-execution and persistent agent state. Use Claude Code if your workflow requires multi-agent orchestration.
