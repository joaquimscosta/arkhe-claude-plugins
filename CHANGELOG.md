...
#### Cross-Platform Plugin Portability (`arkhe/specs/004-cross-platform-portability`)

- **All 14 plugins now installable on Gemini CLI and Codex CLI** alongside Claude Code, from a single repo. Skills are the universal payload (symlinked); commands are transpiled per platform; agents stay Claude-only with inlined-prompt fallback for the 6 subagent-heavy commands.
- **`using-arkhe-skills` bootstrap skill** (in `core` plugin) — maps Claude-only primitives (`AskUserQuestion`, `TaskCreate`, `EnterPlanMode`, the `Skill` tool, the `Agent` tool with `subagent_type`) to Gemini and Codex equivalents at session start.
- **Generated shim trees** at repo root — `.gemini-extensions/<plugin>/` (14 dirs, with `gemini-extension.json`, `GEMINI.md`, `commands/*.toml`, symlinked `skills/`) and `.codex-marketplace/<plugin>/` (14 dirs, with `plugin.json`, `AGENTS.md`, symlinked `skills/`).
- **Build orchestrator** `scripts/build-shims.sh` — single command regenerates all shims for all 14 plugins; idempotent; completes in under 10 s.
- **Per-platform generators** — `scripts/transpile-commands.py` (Claude `.md` → Gemini `.toml`) and `scripts/generate-codex-agents-md.py` (synthesized `AGENTS.md` per plugin, ≤ 32 KiB Codex cap enforced). Stdlib-only Python 3.8+.
- **Per-plugin version sync tooling** — `scripts/bump-version.sh --plugin <name> <version>` keeps each plugin's three manifests (Claude/Gemini/Codex) in lockstep. `--skip-shims` flag for Claude-only releases. `.version-bump.json` enumerates all 42 manifest paths.
- **CI guard** — `.github/workflows/validate-pr.yml` `shim-drift` job re-runs `build-shims.sh` on every PR and fails on any uncommitted regeneration; per-plugin `bump-version.sh --check` catches intra-plugin version drift.
- **`release.sh` integration** — `scripts/release.sh --plugin <name> <version>` now bumps the plugin's three manifests via `bump-version.sh` before tagging. Existing `release.sh <version>` repo-wide flow unchanged.
- **Documentation** — `## Install on Gemini CLI` and `## Install on Codex CLI` sections in all 14 plugin READMEs (generated by `scripts/update-plugin-readmes.py`); Supported Platforms matrix in top-level `README.md`; new Gemini and Codex sections in `INSTALLATION.md`; new "Cross-Platform Notes" section in `CLAUDE.md` under Plugin Architecture.
